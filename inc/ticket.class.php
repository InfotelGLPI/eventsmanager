...
class PluginEventsmanagerTicket extends CommonDBTM {
    ...
    * @param $id
    */
   static function addTicketFromEvent($id) {

      $evt          = new PluginEventsmanagerEvent();
      $ticket       = new Ticket();
      $item         = new Item_Ticket();
      $event_ticket = new PluginEventsmanagerTicket();
 ...
 */
   ...
   function showForTicket($ticket) {
      ...
      if ($number > 0) {
         ...
         foreach ($tickets as $data) {
            ...
            echo "<td>";
            $url = Toolbox::getItemTypeFormURL('PluginEventsmanagerEvent') . "?id=" . $data['id'];
            ...
            $style = "";
            if ($data['eventtype'] > 0) {
               $style = "style='" . PluginEventsmanagerEvent::getTypeColor($data['eventtype']) . "'";
            }
            echo "<td $style>";
            if ($data['eventtype'] > 0) {
               echo PluginEventsmanagerEvent::getEventTypeName($data['eventtype']);
            }
            echo "</td>";
    ...
    * @param array $options
    */
   function showForEvent($ID, $options = []) {
      global $CFG_GLPI;
      ...
      if ($event->fields['status'] < PluginEventsmanagerEvent::CLOSED_STATE
          && $event->fields['status'] > 0) {

         echo "<div class='center'>";
         echo "<form method='post' name='event_form'
         id='event_form'  action='" . Toolbox::getItemTypeFormURL("PluginEventsmanagerEvent") . "'>";
      ...
      $eventsmanager_ticket = new PluginEventsmanagerTicket();
