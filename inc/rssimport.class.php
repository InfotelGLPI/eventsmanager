...
/**
 * Class PluginEventsmanagerRssimport
 */
class PluginEventsmanagerRssimport extends CommonDBTM {
   ...
   function showConfig($idr) {
      ...
      Dropdown::showYesNo('use_with_plugin',
   ...
   static function cronRssImport($task = null) {
      ...
      $rssimport = new PluginEventsmanagerRssimport();
               ...
               if (($feed = RSSFeed::getRSSFeed($rssfeed->fields['url'])) !== false) {
                  foreach ($feed->get_items(0, $rssfeed->fields['max_items']) as $item) {
                     ...
                     if ($data['last_rssfeed_url'] != $item->get_link()) {
                        ...
                        if ($origin->getFromDBByCrit(['itemtype' => PluginEventsmanagerOrigin::RSS,
   ...
   static function getSpecificValueToDisplay($field, $values, array $options = []) {
      ...
      switch ($field) {
         ...
         case 'default_eventtype':
            return PluginEventsmanagerEvent::getEventTypeName($values[$field]);
      }
   ...
   static function getSpecificValueToSelect($field, $name = '', $values = '', array $options = []) {
      ...
      switch ($field) {
         ...
         case 'default_eventtype':
            $options['name']  = $name;
            $options['value'] = $values[$field];
            return PluginEventsmanagerEvent::dropdownType($options);
