...
/**
 * Class PluginEventsmanagerEvent
 */
class PluginEventsmanagerEvent extends CommonDBTM {
   ...
   function defineTabs($options = []) {
      ...
      $this->addStandardTab('PluginEventsmanagerEvent_Item', $ong, $options);
      $this->addStandardTab('PluginEventsmanagerTicket', $ong, $options);
      $this->addStandardTab('KnowbaseItem_Item', $ong, $options);
      //$this->addStandardTab('Notepad', $ong, $options);
      $this->addStandardTab('PluginEventsmanagerEvent_Comment', $ong, $options);
   ...
   function showForm($ID, $options = []) {
      ...
      PluginEventsmanagerEvent_Item::itemAddForm($this, $options);
      ...
      $rand = mt_rand();
      echo "<td>";
      PluginEventsmanagerOrigin::dropdown([
      ...
      Ajax::updateItemOnSelectEvent("dropdown_plugin_eventsmanager_origins_id$rand", "show_items_id$rand",
   ...
   function cleanDBonPurge() {
      ...
      $it = new PluginEventsmanagerEvent_Item();
      $it->deleteByCriteria(['plugin_eventsmanager_events_id' => $this->fields['id']]);

      $ti = new PluginEventsmanagerTicket();
      ...
      if (Session::getCurrentInterface() == 'central') {
         if ($isadmin) {
            ...
            if (Session::haveRight('transfer', READ) && Session::isMultiEntitiesMode()
               ...
               $actions['PluginEventsmanagerEvent' . MassiveAction::CLASS_ACTION_SEPARATOR . 'transfer'] = __('Transfer');
            }
         }
      }
      return $actions;
   ...
   static function processMassiveActionsForOneItemtype(MassiveAction $ma, CommonDBTM $item, array $ids) {
      ...
      switch ($ma->getAction()) {
         case "transfer" :
            $input = $ma->getInput();
            if ($item->getType() == 'PluginEventsmanagerEvent') {
               foreach ($ids as $key) {
                  $item->getFromDB($key);
                  $values["id"]          = $key;
    ...
    * @return int|string
    */
   static function dropdownType(array $options = []) {
    ...
    * @param $value priority ID
    * */
   static function getEventTypeName($value) {
   ...
   static function getTypeColor($val) {
      switch ($val) {
         case 0:
    ...
    *        $val   Id of the event
    * */
   static function getActionAff($val, $value) {
      global $CFG_GLPI;
      switch ($value) {
         default :
            return
