...
/**
 * PluginEventsmanagerEvent_Item Class
 *
 *  Relation between Events and Items
 **/
class PluginEventsmanagerEvent_Item extends CommonDBRelation {

   // From CommonDBRelation
   static public $itemtype_1 = 'PluginEventsmanagerEvent';
   static public $items_id_1 = 'plugin_eventsmanager_events_id';
    ...
    *
    * @param $event PluginEventsmanagerEvent object
    ...
    **/
   static function itemAddForm(PluginEventsmanagerEvent $event, $options = []) {
      global $CFG_GLPI;
      ...
      if ($params['id'] > 0 && $usedcount > 5) {
         echo "<i><a href='" . $event->getFormURL() . "?id=" . $params['id'] . "&amp;forcetab=PluginEventsmanagerEvent_Item$1'>"
   ...
   static function showItemToAdd($plugin_eventsmanager_events_id, $itemtype, $items_id, $options) {
      global $CFG_GLPI;
      ...
      if ($item = $dbu->getItemForItemtype($itemtype)) {
         if ($params['visible']) {
            ...
            if ($params['delete']) {
               $result .= " <span class='fas fa-times-circle pointer' onclick=\"itemAction" . $params['rand'] . "('delete', '$itemtype', '$items_id');\"></span>";
            }
            $result .= "</div>";
         } else {
            $result .= Html::hidden("items_id[$itemtype][$items_id]", ['value' => $items_id]);
    ...
    * @return Nothing (display)
    **/
   static function showForEvent(PluginEventsmanagerEvent $event) {
      global $DB, $CFG_GLPI;
         ...
         switch ($item->getType()) {
            case 'PluginEventsmanagerEvent' :
               if ($_SESSION['glpishow_count_on_tabs']) {
                  $nb = $dbu->countElementsInTable('glpi_plugin_eventsmanager_events_items',
    ...
    * @return type
    */
   static function getUsedItems($events_id) {
         ...
         case 'items_id':
            ...
            if (isset($values['itemtype'])) {
               if (isset($options['comments']) && $options['comments']) {
                  $tmp = Dropdown::getDropdownName($dbu->getTableForItemtype($values['itemtype']),
                                                   $values[$field], 1);
                  return sprintf(__('%1$s %2$s'), $tmp['name'],
                                 Html::showToolTip($tmp['comment'], ['display' => false]));

               }
               return Dropdown::getDropdownName($dbu->getTableForItemtype($values['itemtype']),
                                                $values[$field]);
            }
            break;
      }
